...
Using code from [Lecture 2](), we can load in all the shot event data and create a binary outcome recording whether the shot resulted in a goal.
```{r load-data}
#| output: false
#| message: false
#| warning: false
...
Recall from [Lecture 2]() that a key assumption of XG models is the observed dataset comprises a *sample* from some infinite super-population of shots.
...
As we saw in [Lecture 2](lecture2.qmd#sec-addtl-features), averaging outcomes within bins with very small sample sizes can lead to extreme and erratic estimates.
