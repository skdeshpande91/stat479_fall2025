...
### Review of notation {#sec-notation-review}
...
We can arrange theseindicators into a large $n \times p$ matrix $\boldsymbol{\mathbf{X}}$ whose rows (resp. columns) correspond to stints (resp. players).
...
:::{.callout-important}
## Installing **glmnet**
If you have not already installed **glmnet**, you can do so using
```{r}
...
7. Plots the out-of-sample error as a function of $\lambda.$
...
[^warning]: In fact, the `glmnet()` documentation explicitly warns users from running with only a single value of $\lambda.$
...
4. Like we did in Lecture 3, we create a data frame with every players' regularized adjusted plus/minus value and name
