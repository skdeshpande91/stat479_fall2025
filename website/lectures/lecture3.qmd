...
Like expected goals (XG) from [Lecture 2](), expected runs is a **conditional expectation**. 
...
By the end of this lecture, we will be able to quantify exactly how valuable each of those singles were to his team using run expectancy.
...
<!--
[Last time](lecture2.html) we looked expected goals (XG) in soccer.
At a high-level, XG attempts to answer the question "If a particular shot is replayed over and over again, what proportion of time will it result in a goal?"
Mathematically, XG is  a *conditional expectation*: given a large collection of features $\boldsymbol{\mathbf{x}}$ about a shot, $\textrm{XG}(\boldsymbol{\mathbf{x}}) = \mathbb{E}[Y \vert \boldsymbol{\mathbf{X}} = \boldsymbol{\mathbf{x}}],$ where $Y$ is a binary indicator of whether or not the shot resulted in a goal.
-->
...
We will do this using at-bats taken in the first 8 innings of regular season games played between 2008 and 2023, with the exception of the 2020 season (i.e., all data from the "tracking era" up to but not including the 2024 season).
The 2020 season was dramatically shortened due to the COVID-19 pandemic and featured many rule changes (e.g., seven-inning double headers and new rules for extra innings)
...
<!--
In the code below, we loop over data from the season and compute the number of runs scord by the batting team in the remainder of the half-inning after every at-bat 9 (`RunsRemaining`)
We also compute the number of runs scored during the at-bat (`RunsScored`). 
...
The code below pulls out all pitches thrown in the top of the 8th inning of the .
...
We ultimately want to compute the run value of each at-bat in the 2024 season.
<!--
  1. RunsScore
...
### Appending starting and ending expected runs

Let's look at the Dodgers inning again
...
post_rho <- expected_runs %>% rename(postOuts = Outs, postBaseRunner = BaseRunner, postrho = rho)
