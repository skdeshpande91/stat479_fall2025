...
results <-
  no_ties |>
  dplyr::group_by(Home, Opponent) |> #<1>
  dplyr::summarise(
    HomeWin = sum(Home_Winner), #<2>
    OppWin = sum(Opp_Winner), .groups = 'drop') |> #<2>
  dplyr::mutate(
    Home = factor(Home, levels = sort(unique(c(reg_season$Home, reg_season$Opponent)))), #<3>
    Opponent = factor(Opponent,levels = sort(unique(c(reg_season$Home, reg_season$Opponent))))) #<3>
```
...
#| echo: true
set.seed(123)
results |> dplyr::filter(Home == "Wisconsin") |> dplyr::slice_sample(n=5)
```
...
fit <-
  BradleyTerry2::BTm(
    outcome = cbind(results$HomeWin, results$OppWin), #<1> 
    player1 = Home, player2 = Opponent,data = results) #<2>
```
1. `BTm` wants the counts of home & away team wins for each unique team pair passed as a separate data frame
2. We use `player1` and `player2` to tell `BTm` which columns in the data frame pass with the `data` argument correspond to the home and away team argument
...
Taking a look at the first few rows, we see that Assumption College was taken as the reference team and assigned a latent strength $\lambda = 0.$
...
We see that Wisconsin, Ohio State, Cornell, and Minnesota all have even larger positive estimated strengths.
...
[^frozenfour]: In fact, these four teams reached the semi-finals (or Frozen Four) in the 2025 NCAA Tournament
