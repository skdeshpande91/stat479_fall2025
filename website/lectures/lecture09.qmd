...
Which of the two touchdown plays is more impressive?
  ...
  * The 64-yard touchdown pass from Cooper Rush to Kavonte Turpin ([video link](https://www.youtube.com/watch?v=U0Zp_is-X18))
...
It finally contains a large number of columns that record exactly what happened during the play including the type of play (e.g., run, pass, kickoff, punt, etc.; `play_type`); a narrative description of the play (`desc`); several self-explanatory indicators of what happened (e.g., `fumble`, `complete_pass`, and `passing_yards`). 
...
It turns out that the Herbert-to-McConckey touchdown introduced in @sec-intro was the play with the largest EPA.
...
### Towards partial pooling {#sec-partial-pooling-motivation}
...
### Fitting our multilevel model {#sec-lmer}
...
Each element of the `coef`'s output is a data frame with named rows 
```{r}
#| label: display-coef
