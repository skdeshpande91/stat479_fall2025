...
```
1. 
2. `BTm` uses a somewhat non-standard `formula` interace. This specification tells `BTm()` to estimate a separate latent strength for each team and a parameter for the home advantage.
...
Wisconsin, Ohio State, Cornell, and Minnesota qualified for the semi-finals of the 2025 NCAA Tournament[^frozen4].
The teams were seeded as: (1) Wisconsin; (2) Ohio State; (3) Cornell; and (4) Minnesota.
Following a standard bracket constrution, the first semi-final game (which we will label `SF1`) was played between the 1st and 4th seed (Wisconsin vs Minnesota) and the second semi-final game (which we will label `SF2`) was played between the 2nd and 3rd seed (Ohio State vs Cornell).
...
We can run the simulation 5,000 times.
In each iteration, we will save the eventual winner.
...
for(r in 1:n_sims){
  ...
  finals <- 
    data.frame(Team1 = sf_winners[1], Team2 = sf_winners[2]) |>
    dplyr::left_join(y = seeds |> dplyr::rename(Team1 = Team, Team1.Seed = Seed), 
                     by = "Team1") |>
    dplyr::left_join(y = seeds |> dplyr::rename(Team2 = Team, Team2.Seed = Seed), 
                     by = "Team2") |>
    dplyr::mutate(
      Hi = ifelse(Team1.Seed < Team2.Seed, Team1, Team2),
      Lo = ifelse(Team1.Seed < Team2.Seed, Team2, Team1)) |>
  ...
  final_outcome <- rbinom(n = 1, size = 1, prob = finals$prob)
  if(final_outcome == 1){
    champion[r] <- finals$Hi[1]
  } else{
    champion[r] <- finals$Lo[1]
  }
}
...
compute the probability of every team beating every other team based on our model estimates.
In computing these probabilities, we will take care to 
...
What are the chances that Wisconsin would win the championship?

To build this logic into our simulation, we need to 
