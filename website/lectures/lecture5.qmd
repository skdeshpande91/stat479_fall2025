---
title: "Run expectancy in baseball"
format: html
execute:
  cache: true
---
...
<!--
  ...
  Over the next 3 lectures, we will address this. Here is our plan. Today, we will introduce run value, which is a number that tracks (i) the number of runs scored in each at-bat and (ii) the 
...
---
title: "Lecture 5: Estimating the Expected Value of a Game State"
format: html
execute: 
  cache: true
---
...
atbat2024 <-
  ...
  mutate(end_Outs = last(next_Outs), # goes to last pitch of each at bat and gets next value of variable
         ...
         end_on_1b = last(next_on_1b), # next at-bat
         end_on_2b = last(next_on_2b), # 
         end_on_3b = last(next_on_3b), # e.g., runner on 3rd at start of next at-bat
         end_hc_x = last(hc_x), # hits are on last pitch of at-bat; get x-coordinate
         end_hc_y = last(hc_y), # get y-coordinate of hit from last pitch of at-bat
         end_hit_location = last(hit_location), # get position of hit from last pitch of at-bat
         end_type = last(type),
         end_bb_type = last(bb_type),
         end_events = last(events)
  ...
  select(game_pk, at_bat_number,inning, inning_topbot, 
         ...
         end_type, end_bb_type, end_events,
         des)
```
...
```{r save_atbat}
#| echo: false
save(atbat2024, file = "~/Dropbox/Teaching/2025-26/stat479_f25_sports/lecture_planning/pitchFraming/atbat2024.RData")
