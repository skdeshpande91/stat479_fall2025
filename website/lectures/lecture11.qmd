...
@fig-mccann-castro shows two such pitches.

:::{#fig-mccann-castro layout-ncol=2 fig-align="center"}

![Pitch called a ball](figures/lecture11_mccann.png)
![Pitch called a strike](figures/lecture11_castro.png)
...
But umpire Cooper called the pitch on the left, thrown by Yankees pitcher Masahiro Tanaka, a ball and called the pitch on the right, thrown by Astros pitcher Dallas Keuchel, a strike.
During the television broadcast and on [social media after the game](https://www.crawfishboxes.com/2015/10/8/9483213/astros-pitching-dominates-yankee-hitting-with-better-pitch-framing), many speculated that the reason for this discrepancy in strike zones is due Astros catcher Jason Castro's ability to *frame* pitches, catching them in a way so as to increase the chances of the umpire calling a strike.
...
This time, however, we will need to scrape data f
...
[^inv_logit]: If $u = \log{\frac{p}{1-p}}$ for some $p \in [0,1],$ then $p = 1/[1 + e^{-u}]$, which is monotonically increasing as  
...
#| fig-align: "center"
grid_sep <- 0.05
x_grid <- seq(-2.5, 2.5, by = grid_sep)
...
### A multilevel model {#sec-fit-multilevel}
