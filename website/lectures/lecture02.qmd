...
execute: 
  cache: true
---
## Motivation: Beth Mead's performance at EURO2022

During the EURO2022 football tournament, English player Beth Mead scored 6 goals.
Here are screenshots (and links to videos) of three of those goals.
Which is most impressive to you?

::: {#fig-mead-shots layout-ncol=3 fig-align="center"}
![Mead shot against Austria [link](https://youtu.be/lVGiGZN2gdw?t=1)](figures/mead_austria.png)

![Mead shot against Norway [link](https://youtu.be/lVGiGZN2gdw?t=57)](figures/mead_norway.png)

![Mead shot against Sweden [link](https://youtu.be/lVGiGZN2gdw?t=148)](figures/mead_sweden.png)

:::


We observed several important differences between these shots.
For instance, Mead scored the goal against Austria in a one-on-one situation but had to shoot through several defenders against both Norway and Sweden.
On this basis, should we view 
...
In this lecture, we will introduce the **expected goals** framework, which allows us to *estimate* those long-run frequencies of goals. 
...
As a bit of background, StatsBomb extracts player locations from game film using some pretty interesting computer vision techniques.
...
If that code returns `FALSE`, then you should install the package using the code `devtools::install_github("statsbomb/StatsBombR")`.
...
![Screenshot of the competitions covered in the public data](figures/statsbomb_freecomps.png){width=50% fig-align="center"}
...
#| warning: false
wi_events <-
  StatsBombR::FreeCompetitions() |> # <1> # get table of available competitions
...
### Accounting for additional features{#sec-addtl-features}

For a more refined comparison, we need a finer XG model that conditions on more features, including ones that differ between the two shots.
...
We will explore several such models in [Lecture 3]().
...
So, according to StatsBomb's proprietary model, if Mead repeatedly attempted the three shots from @fig-mead-shots, we should expect them to result in goals 36%, 44%, and 9% of the time.
