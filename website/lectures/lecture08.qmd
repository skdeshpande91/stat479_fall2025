---
title: "Lecture 8: Defensive credit & replacement level"
format: html
execute: 
  cache: true
---
...
[Last lecture](lecture07.qmd), we distributed the run value created in each at-bat between the batter and base runners. 
...
In this lecture, we discuss how to divide this $-\delta$ run value between the pitchers and fielders. 
...
But for at-bats that result with balls put in play, we will divide $-\delta_{i}$ into two parts $\delta^{(p)}_{i} = -\delta \times \hat{p}_{i}$ and $\delta^{(f)} = -\delta_{i} \times (1 - \hat{p}_{i})$ where $\hat{p}_{i}$ is an *estimate* of the probability that play results in an out given the batted ball's location.
We will first allocate 


### Working with hit location data
...
Recall from [Lecture 6](lecture06.qmd) that the StatCast variables `hc_x` and `hc_y` record the coordinates of each batted ball.
...
Luckily, StatCastalso contains the official [fielding position](https://en.wikipedia.org/wiki/Baseball_positions) of the player who first fields the ball[^positions].
When we plot just the pitches first fielded by first basemen, we see that in the original `(hc_x, hc_y)` coordinate system, first base is on the *right* hand side of the plot.
```{r}
#| label: plot-3
#| fig-width: 6
#| fig-height: 6
...
[^positions]: The official position numberings are: 1 (pitcher), 2 (catcher), 3 (first baseman), 4 (second baseman), 5 (third baseman), 6 (shortstop), 7 (left fielder), 8 (center fielder), and 9 (right fielder).
...
[Jim Albert](https://bayesball.github.io), who is one of the founding fathers of Statistical research in baseball, [suggested]() the following transformation from the original `(hc_x, hc_y)` coordinate system to one that places home plate at the bottom of the plot (with coordinates `(0,0)`) and measures distances in feet.
$$
\begin{align}
...
### Generalized additive models {#sec-out-gam}
...
Like we did to predict [XG using shot distance](lecture02.qmd), we can overcome this challenge by building a statistical model.
...
for(i in 1:nrow(grid_probs_bin)){
  rect(xleft = grid_probs_bin$x[i] - grid_sep/2, 
       ...
       xright = grid_probs_bin$x[i] + grid_sep/2,
       ytop = grid_probs_bin$y[i] + grid_sep/2,
       col = adjustcolor(grid_logit_cols[i], alpha.f = 0.5), #<3>
       border = NA)
}
abline(a = 0, b = 1, col = oi_colors[5], lwd = 2) 
abline(a = 0, b = -1, col = oi_colors[5], lwd = 2)
...
3. Unlike with binning-and-averaging, our parametric model is able to make predictions at grid cells not present in the training data. So, we don't need to check whether the predicted probability is `NA`
...
The root cause of the conflict between our data and our model predictions is the model's strong --- and totally unrealistic --- assumption that the log-odds of an out is *linear* in both $x$ and $y.$
...
We can fit GAMs in R using the [**mgcv**](https://cran.r-project.org/web/packages/mgcv/refman/mgcv.html) package.
...
```
After fitting our GAM, we can visualized the estimated out probabilities.
...
While the absolute $RAA$ values are useful on their own, they become even more useful --- and interpretable --- when calibrated to measure performance *relative* to a baseline player.
...
As discussed in class (and also in Section 3.8 of @Baumer2015_openWAR), existing definitions of replacement level are fairly arbitrary.
...
We will instead use @Baumer2015_openWAR's transparent, roster-based definition of replacement level, which is motivated and computed as follows:
