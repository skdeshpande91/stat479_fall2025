...
Over the course of the play, the Chargers offense generated around 8.54 points of EPA.
...
Later, in @sec-replacement, we will define replacement-level players on a position-by-position basis.
...
Although the naming convention is very prevalent within football (i.e, amongs coaches and players), the NFL play-by-play data does not use this convention. 
...
### Constructing composition outcomes
...
In our model, the random intercepts $Q_{q}, C_{c},$ and $D_{d}$ are all modeled as noisy deviations away from league-wise averages $\mu_{Q}, \mu_{C},$ and $\mu_{D}.$
...
To avoid this extra step, we'll continue to use our current[^homework].
...
Burrow's IPAA of 42 points through the air is substantially larger than the next best quarterbacks (Darnlod and Jackson).
...
### Position-specific replacement levels for non-quarterbacks

Although the IPAA values are interesting and informative, they represent a comparion between players and a nebulously defined average.
...
Due to the unique nature of the quarterback position --- virtually *every* offensive play involves runs through the quarterbac --- we need to define "replacement" level somewhat differently.
...
While simple to impelement, this implicitly assumes that every NFL team has at least one non-replacement quarterback.
...
To get this, we load the schedules for the last 10 seasons using the function `load_schedules()` from the **nflreadr** package

<!--
  1. Load all the schedules
  2. For each game, determine who the winner (if result > 0, it's the home team, if result < 0, it's the losing team)
  3. Get score differential for winning team and losing team (-1 * results)
...
skill_war <-
  data.frame(gsis_id = all_players) |>
...
war <-
  ...
  dplyr::left_join(y = run_ipar |> dplyr::select(gsis_id, n_run, ipar_run), by = "gsis_id") |>
  dplyr::left_join(y = receiver_ipar |> dplyr::select(gsis_id, n_rec, ipar_air_rec, ipar_yac_rec), by = "gsis_id") |>
  dplyr::left_join(y = qb_ipar |> dplyr::select(gsis_id, n_pass, n_qbrun, ipar_air_pass, ipar_qbrun), by = "gsis_id")
