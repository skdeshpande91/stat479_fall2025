...
\end{align}
$$
where the errors $\epsilon_{i}$ have mean zero and $\alpha_{g} + \alpha_{e}$ captures the average run value created on at-bats beginning in game state $g$ and resulting in an event $e.$
...
Of course, we don't know the exact values of the $\alpha$'s in this decomposition and will need to estimate them using our data.
...
Similarly, <!-- interpret \beta_{e} -->

In R, to set our baseline, we need to recode `gameState` and `end_events` into *factor* variables.
Then, we will re-level those factors so that 0.000 and field-out are considered to be the reference levels.
Finally, we estimate these betas using ordinary least squares, as implemented with the `lm` function.

For each of the 24 game states and values of `events`, we will model
<!--
 write the relevant LaTeX equation here
-->
...
tmp_df <-
  ...
  dplyr::mutate(
    ...
    end_events = relevel(end_events, ref = "field_out")) |>
  dplyr::select(RunValue, GameState, end_events)
...
We find that many of the coefficients are significantly different than zero[^significance].

[^significance]: Given that we had `{r} nrow(tmp_df)` observations with which to estimate around 40 parameters, this should not be surprising.
```{r}
