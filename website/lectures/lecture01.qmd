...
We've now computed several metrics (FGM, FGP, eFGP, and TSP) that all attempt to quantify an NBA player's shooting ability.

<!--
  What is the purpose a metric in sports analytics? 
  Some are purely there to describe what happened
  
  Others are there to inform what might happen in the future; i.e., what component represents "skill"
  
  Both types of metrics are important
  
  Later we will explore much more formal ways to account for this. 
  
  But in the context of shooting performance, we can at least how repeatable certain values are year-to-year
  
  We find that, among players who attempted at last 400 field goals and at least 100 three point shots, the correlations are pretty consistent. So, these metrics are fairly stable.
  
  Interestingly, TPP is the least
-->
```{r qualified}
qual_box <-
  season_box |>
