...
Finally, in @sec-fundamental, we will lean on some mathematical properties of *absorbing Markov chains* to estimate how many more at-bats a team can expect to face given the current game state.
...
Similarly, if we wanted to know the probability that the half-inning began by visiting the states `0.000`, `0.100`, `0.110`, and `0.111`, we would find all sequences with that starting sub-sequence and add up the corrsponding probabilities.

Of course, calculating the probability for every possible sequence of every possible length is practically impossible.
Luckily, it turns out that the probability distribution of a sequence of random variables $\{X_{t}\}$ is fully determined by the sequence of *conditional* distributions of the form $G_{t} \vert G_{t-1}, \ldots, G_{1}.$
...
@fig-two-state-graph illustrates the graph corresponding to the simple two-state Markov chain from before.

<!--
  Draw something in TiKZ instead of fighting with igraph
  Only use igraph to represent the transition matrix for the actual game out stages
-->
...
@fig-three-state-graph is a graphical illustration of this transition matrix.
...
## Theory of Absorbing Markov Chains {#sec-mc-theory} 
