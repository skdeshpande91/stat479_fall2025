...
In 2017, PithcF/X system was phased out replaced by the radar-based Trackman system, which was originally developed to track the trajectories of golf balls.
...
You can install the package with the code:
```{r}
...
The function [`baseballr::statcast_search()`](https://billpetti.github.io/baseballr/reference/statcast_search.html) allows users query all Statcast data by date, player, or player type.
One of the original **baseballr** authors, Bill Petti, wrote a wrapper function that uses `baseballr::statcast_search()` to pull down an entire season's worth of pitch-by-pitch data; see [this blog post](https://billpetti.github.io/2021-04-02-build-statcast-database-rstats-version-3.0/) for the wrapper function code and [this earlier post](https://billpetti.github.io/2018-02-19-build-statcast-database-rstats/) for details about its design.
...
#| code-fold: true
#| code-summary: "Show the code for Statcast scraper"
#| eval: false
...
The function [`baseballr::chadwick_player_lu()`](https://billpetti.github.io/baseballr/reference/chadwick_player_lu.html) downloads the Chadwick database and stores it as a data table in R.
...
![Play-by-Play from the March 20, 2024 Dodgers-Padres game](figures/lecture3_dodgers_pbp.png){fig-align="center"#fig-dodgers-pbp}
...
## Exercises
  ...
  1. Scrape data from the 2022 and 2023 regular seasons and determine which batters created the most run value.
  2. Compute a new version of expected runs that conditions on the number of outs, baserunner configuration, and ballpark. Using this new version of expected runs, determine which batters create the most run value for their team. 
