...
In [Lecture 7](lecture7.qmd), we will distribute the run value created in every at-bat between the batter and base runners involved in the at-bat.
We will then begin [Lecture 8](lecture8.qmd) by distributing the negative of the run value created by th batting team between the pitcher and fielders involved in each at-bat.
...
player2024_lookup <-
  ...
  dplyr::mutate(FullName = paste(name_first, name_last), # <3>
...
[Later in Lecture 7](lecture7.qmd#sec-rv_position), we will compare each batter's hitting performance to the average performance of other batters who play the same position in the field. 
...
For instance, Ohtani was listed as the Designated Hitter in in that [March 20, 2024] game between the Dodgers and the Padres.
```{r}
...
dodgers_inning <-
  dodgers_inning |>
  dplyr::mutate(RunsRemaining = last(post_bat_score) - bat_score)
```
...
statcast2024 <-
  ...
  dplyr::mutate(RunsRemaining = last(post_bat_score) - bat_score) |> #<3>
  dplyr::ungroup()
```
...
Recall Ohtani's at-bat from the 3rd inning in that March 20, 2024 game against the Padres.
He came up to the plate with 2 outs and no runners on base.
...
As a result of Ohtani's single, he has changed the game state to `Outs = 2` and `BaseRunners=100`.
So, although his at-bat did not directly result in a run scoring, Ohtani *increased* the number of runs his team can expect to score in the remainder of the half-inning.

<!--
Now suppose the batter hits a double, advancing to second base and changing the game state to `Outs=1` and `BaseRunners='100'`
According to our expected runs matrix, from this updated state, the batting team expects to score  runs in the remaining of the half-inning.
...
In [Lecture 7](lecture7.qmd) and [Lecture 8](lecture8.qmd), we will work not only with the game-state (i.e., the values of `Outs` and `BaseRunner`) at the end of every at-bat but also with the actual identities of the base-runners and the location where a ball-in-play was fielded (if the batter hit the ball) at the end of every at-bat.
...
re24 <-
  ...
  dplyr::summarise(RE24 = sum(RunValue),N = n()) |>
