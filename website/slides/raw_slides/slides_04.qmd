...
- Idea: use metrics recorded in box score
  - Points, rebounds, assists, steals, blocks, turnovers
- Advantage: easy to collect, sort, and explain

. . .

- Problem: fails to account for roles
- Problem: should some stats weight more heavily than others?
...
## From points to points/possession

- Comparing totals favors players with more playing time
- APM works with rates: point different per 100 possessions
```{r}
...
- $\alpha_{j}$ captures change in point differential per 100 possessions between
  - Playing 5-on-5 with player $j$ on the court
  - Playing 5-on-4 with player $j$ off the court
...
- Classify some players as "baseline"-level
- **Assumption**: all baseline-level players have common value $\mu$
- Say first $p'$ players are non-baseline. For $j = 1, \ldots, p'$ let $\beta_{j} = \alpha_{j} - \mu$
