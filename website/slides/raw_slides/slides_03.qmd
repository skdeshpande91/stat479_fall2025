...
## Misclassification Rate (Example) {.scrollable}
...
#| eval: true
#| echo: false
simple_preds |> dplyr::select(shot.body_part.name, XG1) |> unique()
...
#| eval: true
#| echo: false
simple_preds |> dplyr::select(shot.body_part.name, shot.technique.name, XG2) |> unique()
```
:::
...
![Left: True function (pink) and a step function approximation (green). Right: representation of the step function as a regression tree](treeapprx_cart.png){width="80%"}
...
cat("RandomForest training logloss:", round(logloss(full_data$Y, preds), digits = 4), "\n")
cat("StatsBomb training logloss:", round(logloss(wi_shots$Y, wi_shots$shot.statsbomb_xg), digits = 4), "\n")
```
...
#| fig-width: 4
#| fig-height: 4
#| fig-cap: Comparing our random forest predictions to StatsBomb's proprietary XG
